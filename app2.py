...
if uploaded:
    ...
    with col_sel1:
        level_sel = st.selectbox("选择问题层级", ["一级问题", "二级问题"], index=0)
    with col_sel2:
        metric_sel = st.selectbox("选择排序指标", ["处理时长_P90", "回复次数_P90"], index=0)
    ...
    if cur_rank.empty:
        ...
        with col1:
            ...
            if not df_rank.empty:
                top5_metric = df_rank.sort_values(metric_sel, ascending=False).head(5)
